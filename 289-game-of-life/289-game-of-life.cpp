...
public:
    ...
    {
        return (x >= 0 && x < board.size() && y >= 0 && y < board[0].size());// inside array condition 
    ...
    { 
        ...
        for (int row = 0; row < board.size(); row++) {
            for (int col = 0; col < board[0].size(); col++) {
                ...
                for (int i = 0; i < 8; i++)
                {
                    int curr_x = row + dx[i], curr_y = col +dy[i]; // get the particular index ;
        ...
        for (int row = 0; row < board.size(); row++) {
            for (int col = 0; col < board[0].size(); col++) {
                if (board[row][col] >= 1)
                    board[row][col] = 1;  // change into live cell
                else
                    board[row][col] = 0; // chagne into dead cell ;
            }
        }
    }
};